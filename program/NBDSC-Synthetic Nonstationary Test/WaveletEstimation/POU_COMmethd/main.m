...
clc;
delt=0;Q=300;
dt=0.001;%鏃堕棿閲囨牱闂撮殧/s
L=35;
fmax=200;%鏈?澶ф樉绀洪鐜?/Hz
fa=pi/2;%相位
seismic_length=800;
...
% Rname='姝ｆ?佸垎甯?';
% r1=r;
original=rand(1,seismic_length);
r1=tan((original-1/2)*pi);
t_seismic=0:dt:(seismic_length-1)*dt;
r=r1/max(abs(r1))/1;
Rname='柯西分布';
r1=r;
save('..\reflection coefficients and wavelet\r1.mat','r1');
%%
...
[wavelet]=Amplitude_To_TimeSequence_phase(dt,amplitude_spectrum_wavelet,fmax,Conv_Length,length(wavelet),fa);
...
Iterations=20;
mu=0;
VecterCut_start=[5;10;20;45];
VecterCut_end=[50;75;100;120];
Vecterp=[0.3;0.3;0.3;0.3];
flip=[1,1,1,1];
...
phase=-pi:pi/180:pi;
[signal_rot,phase_rot]=maxkurt(seismic,phase);
length_wavelet=length(wavelet);
[realx_pou]=Amplitude_To_TimeSequence_phase(dt,RestoringSignal,fmax,Conv_Length,length_wavelet,-phase_rot);
...
% lamda=0.0001;
lamda=0.000001;
W_pou=zeros(length(wavelet)+length(r)-1,length(r));
W_Contraction=zeros(length(wavelet)+length(r)-1,length(r));
for i=1:length(W_pou(1,:))
    W_pou(i:length(wavelet)+i-1,i)=realx_pou;
end
W_pou=W_pou(L+1:end-L,:);
E=eye(size(W_pou));
m_pou=inv(W_pou'*W_pou+E*lamda)*W_pou'*seismic';

for i=1:length(W_Contraction(1,:))
    W_Contraction(i:length(wavelet)+i-1,i)=realx_Contraction;
end
W_Contraction=W_Contraction(L+1:end-L,:);
m_Contraction=inv(W_Contraction'*W_Contraction+E*lamda)*W_Contraction'*seismic';

figure;
subplot(5,1,1);
plot(t_wavelet,wavelet,'b');
title(waveletname);
hold off;
xlabel('\itt/s');
set(gca,'FontName','Times New Roman','FontSize',8,'linewidth',1.5);
set(gca,'TickLength',[0 0.0001]);
subplot(5,1,2);
plot(t_seismic,r,'b');
title('Reflection coefficient sequence');
hold off;
xlabel('\itt/s');
set(gca,'FontName','Times New Roman','FontSize',8,'linewidth',1.5);
set(gca,'TickLength',[0 0.0001]);
% axis([0 t_seismic(end),-1,1])
grid on;
subplot(5,1,3);
plot(t_seismic,seismic,'b');
xlabel('\itt/s');
set(gca,'FontName','Times New Roman','FontSize',8,'linewidth',1.5);
set(gca,'TickLength',[0 0.0001]);
...
grid on;
subplot(5,1,4);
plot(t_seismic,m_Contraction,'b');
xlabel('\itt/s');
set(gca,'FontName','Times New Roman','FontSize',8,'linewidth',1.5);
set(gca,'TickLength',[0 0.0001]);
title('Least squares:COM method');
grid on;
subplot(5,1,5);
plot(t_seismic,m_pou,'b');
xlabel('\itt/s');
set(gca,'FontName','Times New Roman','FontSize',8,'linewidth',1.5);
set(gca,'TickLength',[0 0.0001]);
title('Least squares:POU-COM method');
% axis([0 t_seismic(end),-1,1])
grid on;
...
set(gca,'FontName','Arial','FontSize',12,'linewidth',1.5,'FontWeight','bold');
set(gca,'TickLength',[0 0.001]);
set(gca,'position',[0.09 0.62 0.4 0.364]);
subplot(2,2,2);
hold on;box on;
...
for i=1:length(NormalizedGaborAtomicLibrary(1,:))
    if i>=EffectiveAtoms_Start && i<=EffectiveAtoms_End
        Effective_Gabor_atoms=plot(f_wavelet,0.04*NormalizedGaborAtomicLibrary(TimeOffset+1:TimeOffset+length(f_wavelet),i),'g','LineWidth',1);
    else
        Gabor_atoms=plot(f_wavelet,0.04*NormalizedGaborAtomicLibrary(TimeOffset+1:TimeOffset+length(f_wavelet),i),'k','LineWidth',1);
    end
end
Seismic_wavelet=plot(f_wavelet,amplitude_spectrum_wavelet,'r','LineWidth',1.5);
ylabel('Magnitude');
xlabel({'Frequency/Hz';'(b)'});
xlim([0,200]);
set(gca,'FontName','Arial','FontSize',12,'linewidth',1.5,'FontWeight','bold');
set(gca,'TickLength',[0 0.001])
set(gca,'position',[0.583 0.62 0.4 0.364]);
for i=1:number
    xx=0:0:0;
...
set(gca,'FontName','Arial','FontSize',12,'linewidth',1.5,'FontWeight','bold');
set(gca,'TickLength',[0 0.001]);
set(gca,'position',[0.583 0.11 0.4 0.3861]);
...
Klauder1=wavelet;
%%
save('..\reflection coefficients and wavelet\Ricker1.mat','Ricker1');
save('..\reflection coefficients and wavelet\Ormsby1.mat','Ormsby1');
save('..\reflection coefficients and wavelet\Klauder1.mat','Klauder1');